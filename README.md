...
> **A Mage-OS store is checked as the Magento release it is built on.** Mage-OS locks no `magento/*` package, so its edition, `mage-os/product-community-edition`, is matched against Magento Open Source's advisories at the version in its `extra.magento_version`. A finding is reported against the Mage-OS package and names the Magento version it was matched as. An edition with no `extra.magento_version` is listed as undecided, never as clean.
