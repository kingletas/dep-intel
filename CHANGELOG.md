...
- **A package that replaces a `magento/*` package is checked as that package too.** Every Mage-OS module replaces one, such as `mage-os/framework` for `magento/framework`, at an exact version, and that version is now matched against the replaced package's advisories. A `replace` with `*` means the package was removed, so it is not followed. OSV publishes no advisory under `magento/framework` or any `magento/module-*` name today, so this covers advisories published there in future.
...
- **CI runs `make check` instead of its own list of the same steps.** Pushes and pull requests run on Ubuntu only; the macOS run is still there, on demand, and now installs the linters `make check` runs rather than relying on them being skipped.

### Fixed

- **The `ruff` lane reported green without ever running.** `make lint` printed `ruff  skipped (not installed)` and `make check` went on to report `lint and tests pass`, so every Python module in `bin/dep-intel.d/` went unlinted on any machine without `ruff` on its `PATH`, including every commit made under a pre-commit hook that calls `make check`. A linter that is not installed is a check that did not run, not a check that passed. `make check` now fails, names the linter, and says how to install it. `ruff` is also reached through `uv tool run` when it is not on the `PATH`, which is how it runs on a machine that has `uv` and nothing else, and the result line says which route it took. `shellcheck` had the same silent skip and is fixed with it. `scripts/lint-tool` is the one place that decides this, and the suite asserts both directions with stand-ins so it does not itself depend on a linter being installed.
- **A finding could name a fix you had already passed.** `dep-intel` reported every fix version an advisory had ever published, not the one that applied to the version you run, so a store on Adobe Commerce 2.4.8-p2 was told to upgrade to 2.4.4. That is backwards, through every release fixed since. It happened on 76 of the 165 lines in one scan of a stock Commerce tree. A fix at or below the version you have is no longer offered as one; the report says `no fix published above <your version>` and names the bound that is behind you, which is a different fact from an advisory that publishes no fix at all. The JSON report carries them separately as `fixed_versions` and `superseded_fixes`. A bound that will not parse is still offered. A scanner deciding on its own that a fix it could not read is a fix nobody needs makes the same mistake as one that calls an undecided match clean.
- **An advisory naming your exact version was graded `medium` and worded as if it contradicted itself.** The text read *"advisory lists this exact version although its ranges exclude it"*, which told you to discount the one kind of evidence that is about the build you run. NVD caps a Magento version range at the point where its CPE record stops using ranges and starts listing releases one by one, so the range is the older half of the record and the list is the newer one. An advisory with no ranges at all was already graded `very-high` on the same evidence. It is `very-high` either way now, and says the ranges do not cover the version rather than implying they rule it out. 84 of 165 lines in that same scan carried the wrong grade.
- **An Adobe Commerce store was reported twice for every Adobe advisory.** Commerce ships Magento Open Source inside it at the same version, so a Commerce lockfile holds both metapackages and NVD files one CVE against both CPE products. That turned 65 CVEs into 129 lines, and the two lines disagreed on the remediation because each product caps its range at a different release. One store, one advisory, two answers, and no way to tell which was yours. The advisory is now reported once, against the edition installed, naming the edition it ships. An advisory that reaches only the contained edition is still reported on its own. The relationship is read from the lockfile's own `require`, so editions locked at different versions stay the two separate installs they are.
